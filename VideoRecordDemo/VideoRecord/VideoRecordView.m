...
@interface VideoRecordView()<VideoRecordManagerDelegate>

@property (nonatomic, strong) VideoRecordManager *recorderManager;
...
- (UIView *)recordBtn{
    if (!_recordBtn) {
        ...
        UITapGestureRecognizer* tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(startTakePhoto:)];
        [_recordBtn addGestureRecognizer:tap];
    }
    return _recordBtn;
}

- (void)startTakePhoto:(UITapGestureRecognizer *)gesture {
...
- (void)startRecord:(UILongPressGestureRecognizer *)gesture {
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        NSURL *url = [NSURL fileURLWithPath:[VideoRecordManager cacheFilePath:YES]];
...
#pragma mark - VideoRecordManagerDelegate method
